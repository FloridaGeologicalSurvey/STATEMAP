...
for i in t94:
    thisLine = []
    if len(i[columns["unit"]].strip()) != 0 and i[columns["wnumber"]] not in masterWNum and i[columns["unit"]].strip() != 'avpk':
        shape = i[columns['shape']]
        wnumber = i[columns['wnumber']]
        surf_fm = i[columns['surf']]
        elev = float(i[columns['elev']])
        to_top = float(i[columns['unit']])
...
toPath = os.path.join(outworkspace, "Tap")

arcpy.CreateFeatureclass_management(outworkspace, "Tap", "POINT")
